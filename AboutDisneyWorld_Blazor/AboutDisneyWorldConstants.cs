namespace AboutDisneyWorld_Blazor
{
    namespace AboutDisneyWorldConstants
    {
        public class AboutDisneyWorldConstants
        {
            public const string MainWindowHeader = "Explore Disney World";
            public const string MainWindowTagline = "Disney World is home to four theme parks, two water parks, and Saratoga Springs";

            public const string AnimalKingdomHeader = "Animal Kingdom";
            public const string AnimalKingdomTagline = "Animal Kingdom Park is for the animal lovers";

            public const string EpcotHeader = "Epcot";
            public const string EpcotTagline = "Eat and drink your way around the world";

            public const string HollywoodStudiosHeader = "Hollywood Studios";
            public const string HollywoodStudiosTagline = "Walk down Main Street and pretend you're a movie star";

            public const string MagicKingdomHeader = "Magic Kingdom";
            public const string MagicKingdomTagline = "Heal your inner child and get transported to the most magical place on earth";

            public const string SaratogaSpringsHeader = "Saratoga Springs";
            public const string SaratogaSpringsTagline = "Shop to your legs drop as well as enjoy fine dining and entertainment";
        }
    }
}
